...
function EmbedBlock({ embedUrl, title }: { embedUrl: string; title?: string }) {
  ...
  if (isSpotify) {
    // Click-to-load: activating on click grants user-activation context so the
    // player can autoplay and avoids the "dead play button" issue on desktop.
    if (!playing) {
      return (
        <button
          onClick={() => setPlaying(true)}
          className="w-full rounded-xl border bg-card hover:bg-muted/50 transition-colors p-4 flex items-center gap-4 text-left"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">Tap to load player</p>
          </div>
