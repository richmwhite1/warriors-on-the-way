    ...
    >
      ...
      {post.body && (
        <p className={cn(
          ...
          post.post_type === "reflection" && "italic font-[var(--font-body)]"
        )}>
          {post.body}
        </p>
      )}
...
function EmbedBlock({ embedUrl, title }: { embedUrl: string; title?: string }) {
  ...
  if (isSpotify) {
    ...
    // Load the iframe immediately — no click-to-load gate.
    // Gating via React state mounts/unmounts the iframe which prevents Spotify's JS
    // from initialising correctly in PWA/standalone mode, making the play button
    // unresponsive. loading="lazy" handles bandwidth: the iframe only fetches when
    // near the viewport.
    return (
