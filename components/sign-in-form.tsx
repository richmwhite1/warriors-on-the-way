...
//
// `smsEnabled` mirrors the same gate the rest of the app uses (lib/phone.ts): when
// Twilio isn't configured the phone option is hidden rather than shown and broken.
// It was the only SMS surface that ignored that flag, so it offered a sign-in route
// that could only ever answer "Couldn't send the code."
export function SignInForm({ smsEnabled }: { smsEnabled: boolean }) {
  ...
  return (
    <div className="rounded-2xl border bg-card p-6 space-y-5">
      ...
      {/* Phone OTP — only when we can actually send one */}
      {smsEnabled && (!phoneSent ? (
